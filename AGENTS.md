...
# AI組織構造と各部署の責務境界 (AI Organization & Boundary of Responsibility)
本プロジェクトは、開発規模の拡大に伴うデグレードやコードの競合を防ぐため、仮想的な「AI企業（実働組織）」を形成して運用する。
すべての開発AIは、自身の担当部署の仕様書である `/agents/[部署名]/AGENT.md` を読み込み、その人格と「責務境界（編集可能なファイル範囲）」を厳格に遵守しなければならない。

### 📁 組織フォルダ構成
- `/agents/`
  - `leader/AGENT.md` : AI総監督 / 品質管理部
  - `uiux/AGENT.md` : UI/UXデザイン部 (フロントエンドの見た目と心地よさ)
  - `backend/AGENT.md` : バックエンド開発部 (GAS API・キャッシュ制御・シート操作)
  - `billing/AGENT.md` : ビジネス・Stripe課金部 (決済連携・ライセンス・自動リセット)
  - `qa/AGENT.md` : 品質保証（QA）部 (テスト・検証・例外破壊テスト)
  - `field_ops/AGENT.md` : フィールド・インテリジェンス部 (GPS・地図挙動)
  - `analytics/AGENT.md` : 統計・分析部 (ランキング・実績データ集計)
  - `security/AGENT.md` : セキュリティ・防壁部 (情報漏洩防止・ロック処理・レート制限)
  - `prompt_engineering/AGENT.md` : プロンプトエンジニアリング部 (サブエージェント指示書の作成)

...
- **現在のキャッシュバスター**: `v378`（service-worker.js 更新済み）
- **Gitブランチ**: `main`
...
- **原因**: 
  1. `app.js` 側で `typeof blobToBase64` と `window.` を付けずに参照していたため、グローバルプロパティとしての関数検出が常に `false` となり、Base64の変換・送信がスキップされていた。
  2. GPS取得とカメラ起動を並行して動かしていたため、カメラ（別アプリ）起動中のブラウザサスペンド（一時停止）により、バックグラウンドのGPS取得スレッドがOSによって強制切断（タイムアウト）され、空の座標が送られていた。
- **対策**:
  - **対策1 (app.js)**: `typeof window.blobToBase64 === 'function'` および `window.blobToBase64(...)` に参照を修正し、画像Base64が正しく生成・送信されるように修正。
  - **対策2 (app.js)**: 並行処理を廃止し、「① まずGPSを確実に取得して完了を待つ」→「② 完了後にカメラを起動する」というシーケンシャル（順次）処理に変更。これによりOSサスペンドによるGPSスレッド切断を物理的に100%排除。
  - `index.html` および `service-worker.js` のキャッシュバスターを `v378` にインクリメント。
- **※重要 (Sonnetへの引き継ぎ警告)**: 
  > [!WARNING]
  > 上記の「対策2 (シーケンシャル処理化)」を行った結果、非同期 `await` の遅延によりモバイルブラウザのセキュリティ制限（ユーザーアクション制限）に抵触し、**テンキー確定後にカメラが起動しなくなる不具合**が発生しています。
  > 次回担当の Sonnet は、**即座に `v377` の並行フロー（タップ直後に `capturePhoto` を呼ぶ）に差し戻した上で、`window.blobToBase64` への修正とカメラ復帰後のGPSリトライ（フォールバック）を両立させる実装**を適用してください。

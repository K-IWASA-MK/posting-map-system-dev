...
class DashboardMotion {
  ...
  /**
   * 新着変化差分項目に対するアニメーション・アクセシビリティ適用
   */
  static animateEvolution() {
    const isReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const items = document.querySelectorAll('.evolution-item-new');
    items.forEach(item => {
      if (isReduced) {
        item.style.opacity = '1';
        item.style.transform = 'none';
        item.style.animation = 'none';
        item.classList.remove('evolution-item-new');
        return;
      }
  ...
  /**
   * 新着パターン項目に対するアニメーション・アクセシビリティ適用
   */
  static animatePattern() {
    const isReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const items = document.querySelectorAll('.pattern-item-new');
    items.forEach(item => {
      if (isReduced) {
        item.style.opacity = '1';
        item.style.transform = 'none';
        item.style.animation = 'none';
        item.classList.remove('pattern-item-new');
        return;
      }

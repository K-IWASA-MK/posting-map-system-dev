...
// GAS API エンドポイントは shared/api.js で共通定義される。
// callApi() / callApiPost() は shared/api.js を先に読み込むことでグローバルに展開される。

// callApi() は shared/api.js で定義済み。ここでは定義不要。

// callApiPost() は shared/api.js で定義済み。ここでは定義不要。
...
// オフライン同期は db.js の processQueue() が担当する。
// 旧フロー（syncOfflineQueue）は廃止済み。
...
async function updateRecord(areaName, rowId, isDone, count) {
  ...
  // enqueueSyncが未ロードの場合（通常は発生しない）
  console.error('[updateRecord] enqueueSync is not available. db.js may not be loaded.');
}

// 旧フロー：削除済み
// function saveToOfflineQueue(payload) { ... }
// function applyOptimisticCheck(areaName, rowId, isDone, count) { ... }
// (上記は db.js の IndexedDB フローに統合済み)

window.addEventListener('online', () => {
  // IndexedDB フロー (db.js/processQueue) がオンライン復帰の同期を担当する。
  // processQueue は db.js の window.addEventListener('online') で自動呼び出しされる。
  setSyncStatus('syncing');
  console.log('[app.js] Device is online. db.js processQueue will handle sync.');
});
...
async function saveProfile() {
  ...
  if (!last || !first) {
    ...
    // alert()は使用しない — 入力欄をパルスでフィードバック
    const firstInput = $('user-last')?.value ? $('user-first') : $('user-last');
    if (firstInput) {
      firstInput.style.borderColor = 'rgba(244,63,94,0.8)';
  ...
  try {
    ...
    console.error('[saveProfile] API error:', err.message);
    // alert()は使用しない — ローディングを閉じるのみ（再試行指示不要）
    $('loading').classList.add('opacity-0');
    setTimeout(() => $('loading').classList.add('hidden'), 700);
  }
}
...
async function safeInitApp() {
  ...
  const liffId = "2010374196-gIYb6PDH";

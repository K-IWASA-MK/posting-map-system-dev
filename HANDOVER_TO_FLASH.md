Created At: 2026-05-26T08:51:00Z
Completed At: 2026-05-26T08:51:00Z
File Path: `file:///Volumes/SSD_DATA/posting-map-system/HANDOVER_TO_FLASH.md`

# 開発引き継ぎ事項 (更新: 2026-05-26 v263)
...
フロントエンド（GitHub Pages）へ適用・デプロイ準備済みです。キャッシュバスターは **v263** です。

### ① 起動時スプラッシュ画面のチラつきテキストを消去 (`index.html`, `app.js`)
- `index.html` 内のゲートウェイ画面 (`screen-gateway`) のタイトルである `gateway-title` の初期テキスト（「自動ログイン処理中...」）を完全に削除して空欄にしました。これにより、キャッシュによる起動時の一瞬の文字のチラつきを完全に防止しました。
...
- **キャッシュバスター**: 現在 `v263`。フロントエンドファイルを変更する際は必ず `index.html` のバージョン番号を上げてください。

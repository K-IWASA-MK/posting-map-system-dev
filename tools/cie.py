...
def run_runtime_event_execution_log_provider(args):
    """
    runtime-event-execution-log-provider サブコマンド: EventExecutionLogProviderManager を使用して
    runtime_event_execution_log_provider.json を生成する。
    注意: この runtime_event_execution_log_bridge.json から直接復元するデータフローは、
    将来的な Runtime Bridge Layer との完全統合を見据えた「暫定・テスト用入力」としての実装です。
    """
    import sys
    import json
    ...
    try:
        from plugin_platform.plugin.runtime_adapter import RuntimeContext
        from plugin_platform.plugin.runtime_event_execution_log_bridge import (
            RuntimeEventExecutionLogBridge,
            RuntimeExecutionLogBridge
        )
        from plugin_platform.plugin.runtime_event_execution_log_provider import EventExecutionLogProviderManager
    except ImportError as e:
        print(f"Error: Failed to import execution log provider modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    bridge_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_bridge.json")
    if not os.path.exists(bridge_path):
        print(f"Error: Bridge event execution log result not found at {bridge_path}. Please run 'runtime-event-execution-log-bridge' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution log bridge: {e}", file=sys.stderr)
        sys.exit(3)
        
    bridge_rec = bridge_data.get("bridge_record", {})
    execution_id = bridge_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # 直近の Bridge オブジェクトだけを Python クラスとして復元し、
    # それより下位の階層（Adapter以下）は辞書のまま DTO に引き渡す。
    bridge_part = bridge_rec.get("bridge", {})
    bridge_obj = RuntimeExecutionLogBridge(
        bridge_id=bridge_part.get("bridge_id"),
        adapter_id=bridge_part.get("adapter_id"),
        runtime_type=bridge_part.get("runtime_type"),
        bridge_target=bridge_part.get("bridge_target"),
        bridge_state=bridge_part.get("bridge_state"),
        bridge_version=bridge_part.get("bridge_version"),
        bridge_map=bridge_part.get("bridge_map", []),
        metadata=bridge_part.get("metadata", {}),
        trace_id=bridge_part.get("trace_id")
    )
    
    execution_log_bridge_obj = RuntimeEventExecutionLogBridge(
        bridge_id=bridge_rec.get("bridge_id"),
        # 下位は辞書(dict)のまま引き渡す
        runtime_event_execution_log_adapter=bridge_rec.get("runtime_event_execution_log_adapter", {}),
        bridge=bridge_obj,
        metadata=bridge_rec.get("metadata", {}),
        trace_id=bridge_rec.get("trace_id")
    )
    ...
    context = RuntimeContext(
        runtime_id="system_executionlogprovider_context",
        configuration=configuration,
        environment=environment,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        provider_execution_obj = EventExecutionLogProviderManager.create_execution_provider(execution_log_bridge_obj, context)
    except AssertionError as e:
        print(f"Assertion Error during execution log provider create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_provider.json")
    ...
    provider_data = {
        ...
        "provider_record": provider_execution_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Log Provider Execution (Dry Run)")
        print(f"Provider ID: {provider_execution_obj.provider_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(provider_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Log Provider Execution successfully written to runtime_event_execution_log_provider.json")
        sys.exit(0)
    except IOError as e:
        print(f"Error: Failed to write runtime_event_execution_log_provider.json: {e}", file=sys.stderr)
        sys.exit(3)

def run_runtime_event_execution_log_instance(args):
    """
    runtime-event-execution-log-instance サブコマンド: EventExecutionLogInstanceManager を使用して
    runtime_event_execution_log_instance.json を生成する。
    注意: この runtime_event_execution_log_provider.json から直接復元するデータフローは、
    将来的な Runtime Provider Layer との完全統合を見据えた「暫定・テスト用入力」としての実装です。
    """
    import sys
    import json
    ...
    try:
        from plugin_platform.plugin.runtime_adapter import RuntimeContext
        from plugin_platform.plugin.runtime_event_execution_log_provider import (
            RuntimeEventExecutionLogProvider,
            RuntimeExecutionLogProvider
        )
        from plugin_platform.plugin.runtime_event_execution_log_instance import EventExecutionLogInstanceManager
    except ImportError as e:
        print(f"Error: Failed to import execution log instance modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    provider_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_provider.json")
    if not os.path.exists(provider_path):
        print(f"Error: Provider event execution log result not found at {provider_path}. Please run 'runtime-event-execution-log-provider' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution log provider: {e}", file=sys.stderr)
        sys.exit(3)
        
    provider_rec = provider_data.get("provider_record", {})
    execution_id = provider_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # 直近の Provider オブジェクトだけを Python クラスとして復元し、
    # それより下位の階層（Bridge以下）は辞書のまま DTO に引き渡す。
    provider_part = provider_rec.get("provider", {})
    provider_obj = RuntimeExecutionLogProvider(
        provider_id=provider_part.get("provider_id"),
        bridge_id=provider_part.get("bridge_id"),
        runtime_type=provider_part.get("runtime_type"),
        provider_type=provider_part.get("provider_type"),
        provider_state=provider_part.get("provider_state"),
        provider_version=provider_part.get("provider_version"),
        provider_map=provider_part.get("provider_map", []),
        metadata=provider_part.get("metadata", {}),
        trace_id=provider_part.get("trace_id")
    )
    
    execution_log_provider_obj = RuntimeEventExecutionLogProvider(
        provider_id=provider_rec.get("provider_id"),
        # 下位は辞書(dict)のまま引き渡す
        runtime_event_execution_log_bridge=provider_rec.get("runtime_event_execution_log_bridge", {}),
        provider=provider_obj,
        metadata=provider_rec.get("metadata", {}),
        trace_id=provider_rec.get("trace_id")
    )
    ...
    context = RuntimeContext(
        runtime_id="system_executionloginstance_context",
        configuration=configuration,
        environment=environment,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        instance_execution_obj = EventExecutionLogInstanceManager.create_execution_instance(execution_log_provider_obj, context)
    except AssertionError as e:
        print(f"Assertion Error during execution log instance create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_instance.json")
    ...
    instance_data = {
        ...
        "instance_record": instance_execution_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Log Instance Execution (Dry Run)")
        print(f"Instance ID: {instance_execution_obj.instance_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(instance_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Log Instance Execution successfully written to runtime_event_execution_log_instance.json")
        sys.exit(0)
    except IOError as e:
        print(f"Error: Failed to write runtime_event_execution_log_instance.json: {e}", file=sys.stderr)
        sys.exit(3)

def run_runtime_event_execution_log_session(args):
    """
    runtime-event-execution-log-session サブコマンド: EventExecutionLogSessionManager を使用して
    runtime_event_execution_log_session.json を生成する。
    注意: この runtime_event_execution_log_instance.json から直接復元するデータフローは
    将来的な Runtime Instance Layer との完全統合を見据えた「暫定・テスト用入力」としての実装です。
    """
    import sys
    import json
    ...
    try:
        from plugin_platform.plugin.runtime_adapter import RuntimeContext
        from plugin_platform.plugin.runtime_event_execution_log_instance import (
            RuntimeEventExecutionLogInstance,
            RuntimeExecutionLogInstance
        )
        from plugin_platform.plugin.runtime_event_execution_log_session import EventExecutionLogSessionManager
    except ImportError as e:
        print(f"Error: Failed to import execution log session modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    instance_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_instance.json")
    if not os.path.exists(instance_path):
        print(f"Error: Instance event execution log result not found at {instance_path}. Please run 'runtime-event-execution-log-instance' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution log instance: {e}", file=sys.stderr)
        sys.exit(3)
        
    instance_rec = instance_data.get("instance_record", {})
    execution_id = instance_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # 直近 of Instance DTO のみ復元し、下位は辞書(dict)のまま渡す。
    instance_part = instance_rec.get("instance", {})
    instance_obj = RuntimeExecutionLogInstance(
        instance_id=instance_part.get("instance_id"),
        provider_id=instance_part.get("provider_id"),
        runtime_type=instance_part.get("runtime_type"),
        instance_type=instance_part.get("instance_type"),
        instance_state=instance_part.get("instance_state"),
        instance_version=instance_part.get("instance_version"),
        instance_map=instance_part.get("instance_map", []),
        metadata=instance_part.get("metadata", {}),
        trace_id=instance_part.get("trace_id")
    )
    
    execution_log_instance_obj = RuntimeEventExecutionLogInstance(
        instance_id=instance_rec.get("instance_id"),
        runtime_event_execution_log_provider=instance_rec.get("runtime_event_execution_log_provider", {}),
        instance=instance_obj,
        metadata=instance_rec.get("metadata", {}),
        trace_id=instance_rec.get("trace_id")
    )
    ...
    context = RuntimeContext(
        runtime_id="system_executionlogsession_context",
        configuration=configuration,
        environment=environment,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        session_execution_obj = EventExecutionLogSessionManager.create_execution_session(execution_log_instance_obj, context)
    except AssertionError as e:
        print(f"Assertion Error during execution log session create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_session.json")
    ...
    session_data = {
        ...
        "session_record": session_execution_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Log Session Execution (Dry Run)")
        print(f"Session ID: {session_execution_obj.session_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(session_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Log Session Execution successfully written to runtime_event_execution_log_session.json")
        sys.exit(0)
    except IOError as e:
        print(f"Error: Failed to write runtime_event_execution_log_session.json: {e}", file=sys.stderr)
        sys.exit(3)

def run_runtime_event_execution_log_environment(args):
    """
    runtime-event-execution-log-environment サブコマンド: EventExecutionLogEnvironmentManager を使用して
    runtime_event_execution_log_environment.json を生成する。
    注意: この runtime_event_execution_log_session.json から直接復元するデータフローは
    将来的な Runtime Session Layer との完全統合を見据えた「暫定・テスト用入力」としての実装です。
    """
    import sys
    import json
    ...
    try:
        from plugin_platform.plugin.runtime_adapter import RuntimeContext
        from plugin_platform.plugin.runtime_event_execution_log_session import (
            RuntimeEventExecutionLogSession,
            RuntimeExecutionLogSession
        )
        from plugin_platform.plugin.runtime_event_execution_log_environment import EventExecutionLogEnvironmentManager
    except ImportError as e:
        print(f"Error: Failed to import execution log environment modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    session_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_session.json")
    if not os.path.exists(session_path):
        print(f"Error: Session event execution log result not found at {session_path}. Please run 'runtime-event-execution-log-session' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution log session: {e}", file=sys.stderr)
        sys.exit(3)
        
    session_rec = session_data.get("session_record", {})
    execution_id = session_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # 直近 of Session DTO のみ復元し、下位は辞書(dict)のまま渡す。
    session_part = session_rec.get("session", {})
    session_obj = RuntimeExecutionLogSession(
        session_id=session_part.get("session_id"),
        instance_id=session_part.get("instance_id"),
        runtime_type=session_part.get("runtime_type"),
        session_type=session_part.get("session_type"),
        session_state=session_part.get("session_state"),
        session_version=session_part.get("session_version"),
        session_map=session_part.get("session_map", []),
        metadata=session_part.get("metadata", {}),
        trace_id=session_part.get("trace_id")
    )
    
    execution_log_session_obj = RuntimeEventExecutionLogSession(
        session_id=session_rec.get("session_id"),
        runtime_event_execution_log_instance=session_rec.get("runtime_event_execution_log_instance", {}),
        session=session_obj,
        metadata=session_rec.get("metadata", {}),
        trace_id=session_rec.get("trace_id")
    )
    ...
    context = RuntimeContext(
        runtime_id="system_executionlogenvironment_context",
        configuration=configuration,
        environment=environment,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        environment_execution_obj = EventExecutionLogEnvironmentManager.create_execution_environment(execution_log_session_obj, context)
    except AssertionError as e:
        print(f"Assertion Error during execution log environment create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_environment.json")
    ...
    environment_data = {
        ...
        "environment_record": environment_execution_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Log Environment Execution (Dry Run)")
        print(f"Environment ID: {environment_execution_obj.environment_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(environment_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Log Environment Execution successfully written to runtime_event_execution_log_environment.json")
        sys.exit(0)
    except IOError as e:
...
def run_runtime_event_execution_log_resource(args):
    """
    runtime-event-execution-log-resource サブコマンド: EventExecutionLogResourceManager を使用して
    runtime_event_execution_log_resource.json を生成する。
    注意: この runtime_event_execution_log_workspace.json から直接復元するデータフローは
    将来的な Runtime Workspace Layer との完全統合を見据えた「暫定・テスト用入力」としての実装です。
    """
    import sys
    import json
    ...
    try:
        from plugin_platform.plugin.runtime_adapter.runtime_context import RuntimeRuntime
        from plugin_platform.plugin.runtime_event_execution_log_workspace import (
            RuntimeEventExecutionLogWorkspace,
            RuntimeExecutionLogWorkspace
        )
        from plugin_platform.plugin.runtime_event_execution_log_resource import EventExecutionLogResourceManager
    except ImportError as e:
        print(f"Error: Failed to import execution log resource modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    workspace_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_workspace.json")
    if not os.path.exists(workspace_path):
        print(f"Error: Workspace event execution log result not found at {workspace_path}. Please run 'runtime-event-execution-log-workspace' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution log workspace: {e}", file=sys.stderr)
        sys.exit(3)
        
    workspace_rec = workspace_data.get("workspace_record", {})
    execution_id = workspace_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # 直近 of Workspace DTO のみ復元し、下位は辞書(dict)のまま渡す。
    workspace_part = workspace_rec.get("workspace", {})
    workspace_obj = RuntimeExecutionLogWorkspace(
        workspace_id=workspace_part.get("workspace_id"),
        environment_id=workspace_part.get("environment_id"),
        runtime_type=workspace_part.get("runtime_type"),
        workspace_type=workspace_part.get("workspace_type"),
        workspace_state=workspace_part.get("workspace_state"),
        workspace_version=workspace_part.get("workspace_version"),
        workspace_map=workspace_part.get("workspace_map", []),
        metadata=workspace_part.get("metadata", {}),
        trace_id=workspace_part.get("trace_id")
    )
    
    execution_log_workspace_obj = RuntimeEventExecutionLogWorkspace(
        workspace_id=workspace_rec.get("workspace_id"),
        runtime_event_execution_log_environment=workspace_rec.get("runtime_event_execution_log_environment", {}),
        workspace=workspace_obj,
        metadata=workspace_rec.get("metadata", {}),
        trace_id=workspace_rec.get("trace_id")
    )
    ...
    # No Context Leak: 引数と型に RuntimeRuntime を使用
    runtime_system = RuntimeRuntime(
        runtime_id="system_executionlogresource_runtime",
        configuration=configuration,
        environment=environment_name,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        resource_execution_obj = EventExecutionLogResourceManager.create_execution_resource(execution_log_workspace_obj, runtime_system)
    except AssertionError as e:
        print(f"Assertion Error during execution log resource create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_resource.json")
    ...
    resource_data = {
        ...
        "resource_record": resource_execution_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Log Resource Execution (Dry Run)")
        print(f"Resource ID: {resource_execution_obj.resource_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(resource_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Log Resource Execution successfully written to runtime_event_execution_log_resource.json")
        sys.exit(0)
    except IOError as e:
        print(f"Error: Failed to write runtime_event_execution_log_resource.json: {e}", file=sys.stderr)
        sys.exit(3)

def run_runtime_event_execution_log_registry(args):
    """
    runtime-event-execution-log-registry サブコマンド: EventExecutionLogRegistryManager を使用して
    runtime_event_execution_log_registry.json を生成する。
    注意: この runtime_event_execution_log_resource.json から直接復元するデータフローは
    将来的な Runtime Resource Layer との完全統合を見据えた「暫定・テスト用入力」としての実装です。
    """
    import sys
    import json
    ...
    try:
        from plugin_platform.plugin.runtime_adapter.runtime_context import RuntimeRuntime
        from plugin_platform.plugin.runtime_event_execution_log_resource import (
            RuntimeEventExecutionLogResource,
            RuntimeExecutionLogResource
        )
        from plugin_platform.plugin.runtime_event_execution_log_registry import EventExecutionLogRegistryManager
    except ImportError as e:
        print(f"Error: Failed to import execution log registry modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    resource_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_resource.json")
    if not os.path.exists(resource_path):
        print(f"Error: Resource event execution log result not found at {resource_path}. Please run 'runtime-event-execution-log-resource' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution log resource: {e}", file=sys.stderr)
        sys.exit(3)
        
    resource_rec = resource_data.get("resource_record", {})
    execution_id = resource_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # 直近 of Resource DTO のみ復元し、下位は辞書(dict)のまま渡す。
    resource_part = resource_rec.get("resource", {})
    resource_obj = RuntimeExecutionLogResource(
        resource_id=resource_part.get("resource_id"),
        workspace_id=resource_part.get("workspace_id"),
        runtime_type=resource_part.get("runtime_type"),
        resource_type=resource_part.get("resource_type"),
        resource_state=resource_part.get("resource_state"),
        resource_version=resource_part.get("resource_version"),
        resource_map=resource_part.get("resource_map", []),
        metadata=resource_part.get("metadata", {}),
        trace_id=resource_part.get("trace_id")
    )
    
    execution_log_resource_obj = RuntimeEventExecutionLogResource(
        resource_id=resource_rec.get("resource_id"),
        runtime_event_execution_log_workspace=resource_rec.get("runtime_event_execution_log_workspace", {}),
        resource=resource_obj,
        metadata=resource_rec.get("metadata", {}),
        trace_id=resource_rec.get("trace_id")
    )
    ...
    # No Context Leak: 引数と型に RuntimeRuntime を使用
    runtime_system = RuntimeRuntime(
        runtime_id="system_executionlogregistry_runtime",
        configuration=configuration,
        environment=environment_name,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        registry_execution_obj = EventExecutionLogRegistryManager.create_execution_registry(execution_log_resource_obj, runtime_system)
    except AssertionError as e:
        print(f"Assertion Error during execution log registry create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_registry.json")
    ...
    registry_data = {
        ...
        "registry_record": registry_execution_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Log Registry Execution (Dry Run)")
        print(f"Registry ID: {registry_execution_obj.registry_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(registry_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Log Registry Execution successfully written to runtime_event_execution_log_registry.json")
        sys.exit(0)
    except IOError as e:
        print(f"Error: Failed to write runtime_event_execution_log_registry.json: {e}", file=sys.stderr)
        sys.exit(3)

def run_runtime_event_execution_log_repository(args):
    """
    runtime-event-execution-log-repository サブコマンド: EventExecutionLogRepositoryManager を使用して
    runtime_event_execution_log_repository.json を生成する。
    注意: この runtime_event_execution_log_registry.json から直接復元するデータフローは
    将来的な Runtime Registry Layer との完全統合を見据えた「暫定・テスト用入力」としての実装です。
    """
    import sys
    import json
    ...
    try:
        from plugin_platform.plugin.runtime_adapter.runtime_context import RuntimeRuntime
        from plugin_platform.plugin.runtime_event_execution_log_registry import (
            RuntimeEventExecutionLogRegistry,
            RuntimeExecutionLogRegistry
        )
        from plugin_platform.plugin.runtime_event_execution_log_repository import EventExecutionLogRepositoryManager
    except ImportError as e:
        print(f"Error: Failed to import execution log repository modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    registry_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_registry.json")
    if not os.path.exists(registry_path):
        print(f"Error: Registry event execution log result not found at {registry_path}. Please run 'runtime-event-execution-log-registry' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution log registry: {e}", file=sys.stderr)
        sys.exit(3)
        
    registry_rec = registry_data.get("registry_record", {})
    execution_id = registry_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # 直近 of Registry DTO のみ復元し、下位は辞書(dict)のまま渡す。
    registry_part = registry_rec.get("registry", {})
    registry_obj = RuntimeExecutionLogRegistry(
        registry_id=registry_part.get("registry_id"),
        resource_id=registry_part.get("resource_id"),
        runtime_type=registry_part.get("runtime_type"),
        registry_type=registry_part.get("registry_type"),
        registry_state=registry_part.get("registry_state"),
        registry_version=registry_part.get("registry_version"),
        registry_map=registry_part.get("registry_map", []),
        metadata=registry_part.get("metadata", {}),
        trace_id=registry_part.get("trace_id")
    )
    
    execution_log_registry_obj = RuntimeEventExecutionLogRegistry(
        registry_id=registry_rec.get("registry_id"),
        runtime_event_execution_log_resource=registry_rec.get("runtime_event_execution_log_resource", {}),
        registry=registry_obj,
        metadata=registry_rec.get("metadata", {}),
        trace_id=registry_rec.get("trace_id")
    )
    ...
    # No Context Leak: 引数と型に RuntimeRuntime を使用
    runtime_system = RuntimeRuntime(
        runtime_id="system_executionlogrepository_runtime",
        configuration=configuration,
        environment=environment_name,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        repository_execution_obj = EventExecutionLogRepositoryManager.create_execution_repository(execution_log_registry_obj, runtime_system)
    except AssertionError as e:
        print(f"Assertion Error during execution log repository create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_event_execution_log_repository.json")
    ...
    repository_data = {
        ...
        "repository_record": repository_execution_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Log Repository Execution (Dry Run)")
        print(f"Repository ID: {repository_execution_obj.repository_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(repository_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Log Repository Execution successfully written to runtime_event_execution_log_repository.json")
        sys.exit(0)
    except IOError as e:
...
def run_runtime_event_execution_engine(args):
    """
    runtime-event-execution-engine サブコマンド: EngineManager を使用して
    runtime_event_execution_engine.json を生成する。
    """
    import sys
    import json
    ...
    blueprint_path = os.path.join(script_dir, "plugins", "runtime_event_execution_blueprint.json")
    if not os.path.exists(blueprint_path):
        print(f"Error: Blueprint event execution result not found at {blueprint_path}. Please run 'runtime-event-execution-blueprint' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution blueprint: {e}", file=sys.stderr)
        sys.exit(3)
        
    blueprint_rec = blueprint_data.get("blueprint_record", {})
    execution_id = blueprint_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計】
    # RuntimeEventExecutionBlueprint までDTO復元し、下位は dict 保持
    blueprint_obj = RuntimeEventExecutionBlueprint.from_dict(blueprint_rec)
    ...
    runtime_system = RuntimeRuntime(
        runtime_id="system_executionengine_runtime",
        configuration=configuration,
        environment=environment_name,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        engine_obj = EngineManager.create_execution_engine(blueprint_obj, runtime_system)
    except AssertionError as e:
        print(f"Assertion Error during execution engine create: {e}", file=sys.stderr)
        sys.exit(3)
...
def run_runtime_execution_runtime(args):
    """
    runtime-execution-runtime サブコマンド: RuntimeExecutionRuntimeManager を使用して
    runtime_execution_runtime.json を生成する。
    """
    import sys
    import json
    ...
    try:
        ...
        print(f"Error: Failed to import execution runtime modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    engine_path = os.path.join(script_dir, "plugins", "runtime_event_execution_engine.json")
    if not os.path.exists(engine_path):
        print(f"Error: Engine event execution result not found at {engine_path}. Please run 'runtime-event-execution-engine' first.", file=sys.stderr)
        sys.exit(3)
    ...
    # 【簡素化復元設計】
    # RuntimeEventExecutionEngine までDTO復元し、下位は dict 保持
    engine_obj = RuntimeEventExecutionEngine.from_dict(engine_rec)
    ...
    runtime_system = RuntimeRuntime(
        runtime_id="system_executionruntime_runtime",
        configuration=configuration,
        environment=environment_name,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        runtime_obj = RuntimeExecutionRuntimeManager.create_execution_runtime(engine_obj, runtime_system)
    except AssertionError as e:
        print(f"Assertion Error during execution runtime create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_execution_runtime.json")
    ...
    runtime_data = {
        ...
        "runtime_record": runtime_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Runtime (Dry Run)")
        print(f"Runtime ID: {runtime_obj.runtime_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(runtime_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Runtime successfully written to runtime_execution_runtime.json")
        sys.exit(0)
    except IOError as e:
        print(f"Error: Failed to write runtime_execution_runtime.json: {e}", file=sys.stderr)
        sys.exit(3)

def run_runtime_execution_pipeline(args):
    """
    runtime-execution-pipeline サブコマンド: RuntimeExecutionPipelineManager を使用して
    runtime_execution_pipeline.json を生成する。
    """
    import sys
    import json
    ...
    try:
        ...
        print(f"Error: Failed to import execution pipeline modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    runtime_path = os.path.join(script_dir, "plugins", "runtime_execution_runtime.json")
    if not os.path.exists(runtime_path):
        print(f"Error: Runtime event execution runtime result not found at {runtime_path}. Please run 'runtime-execution-runtime' first.", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"Error: Failed to load runtime event execution runtime: {e}", file=sys.stderr)
        sys.exit(3)
    ...
    # 【簡素化復元設計】
    # RuntimeExecutionRuntime までDTO復元し、下位は dict 保持
    runtime_obj = RuntimeExecutionRuntime.from_dict(runtime_rec)
    ...
    runtime_system = RuntimeRuntime(
        runtime_id="system_executionpipeline_runtime",
        configuration=configuration,
        environment=environment_name,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        pipeline_obj = RuntimeExecutionPipelineManager.create_execution_pipeline(runtime_obj, runtime_system)
    except AssertionError as e:
        print(f"Assertion Error during execution pipeline create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_execution_pipeline.json")
    ...
    pipeline_data = {
        ...
        "pipeline_record": pipeline_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Pipeline (Dry Run)")
        print(f"Pipeline ID: {pipeline_obj.pipeline_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(pipeline_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Pipeline successfully written to runtime_execution_pipeline.json")
        sys.exit(0)
    except IOError as e:
...
def run_runtime_execution_controller(args):
    """
    runtime-execution-controller サブコマンド: RuntimeExecutionControllerManager を使用して
    runtime_execution_controller.json を生成する。
    """
    import sys
    import json
    ...
    try:
        ...
        print(f"Error: Failed to import execution controller modules: {e}", file=sys.stderr)
        sys.exit(3)
        
    orch_path = os.path.join(script_dir, "plugins", "runtime_execution_orchestrator.json")
    if not os.path.exists(orch_path):
        print(f"Error: Runtime event execution orchestrator result not found at {orch_path}. Please run 'runtime-execution-orchestrator' first.", file=sys.stderr)
        sys.exit(3)
    ...
    orch_rec = orch_data.get("orchestrator_record", {})
    execution_id = orch_data.get("_meta", {}).get("execution_id", "session_cie_default")
    
    # 【簡素化復元設計 & CLI復元ルール】
    # RuntimeExecutionOrchestrator のみ DTO復元対象とし、それ以外は完全に dict 固定。
    # ExecutionPlan / ExecutionGraph / ExecutionPipeline / RuntimeDefinition は絶対に DTO化しない。
    orch_obj = RuntimeExecutionOrchestrator.from_dict(orch_rec)
    ...
    runtime_system = RuntimeRuntime(
        runtime_id="system_executioncontroller_runtime",
        configuration=configuration,
        environment=environment_name,
        variables=variables,
        metadata={"version": 1}
    ...
    try:
        controller_obj = RuntimeExecutionControllerManager.create_execution_controller(orch_obj, runtime_system)
    except AssertionError as e:
        print(f"Assertion Error during execution controller create: {e}", file=sys.stderr)
        sys.exit(3)
        
    output_path = os.path.join(script_dir, "plugins", "runtime_execution_controller.json")
    ...
    controller_data = {
        ...
        "controller_record": controller_obj.to_dict()
    }
    
    if args.dry_run:
        print("Plugin Runtime Session Event Execution Controller (Dry Run)")
        print(f"Controller ID: {controller_obj.controller_id}")
        sys.exit(0)
        
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(controller_data, f, indent=2, ensure_ascii=False)
        print("Plugin Runtime Session Event Execution Controller successfully written to runtime_execution_controller.json")
        sys.exit(0)
    except IOError as e:
...
Available commands:
  ...
  runtime-event-metadata Manage plugin runtime session event metadata.
    ...
    # runtime-event-execution-log-provider コマンドパーサー
    runtime_event_execution_log_provider_parser = subparsers.add_parser("runtime-event-execution-log-provider", help="Manage plugin runtime session event execution log execution provider")
    runtime_event_execution_log_provider_parser.add_argument("--dry-run", action="store_true", help="Perform dry-run without writing execution log provider result")
    
    # runtime-event-execution-log-instance コマンドパーサー
    runtime_event_execution_log_instance_parser = subparsers.add_parser("runtime-event-execution-log-instance", help="Manage plugin runtime session event execution log execution instance")
    runtime_event_execution_log_instance_parser.add_argument("--dry-run", action="store_true", help="Perform dry-run without writing execution log instance result")
    
    # runtime-event-execution-log-session コマンドパーサー
    runtime_event_execution_log_session_parser = subparsers.add_parser("runtime-event-execution-log-session", help="Manage plugin runtime session event execution log execution session")
    runtime_event_execution_log_session_parser.add_argument("--dry-run", action="store_true", help="Perform dry-run without writing execution log session result")
    
    # runtime-event-execution-log-environment コマンドパーサー
    ...
    # runtime-event-execution-log-resource コマンドパーサー
    runtime_event_execution_log_resource_parser = subparsers.add_parser("runtime-event-execution-log-resource", help="Manage plugin runtime session event execution log execution resource")
    runtime_event_execution_log_resource_parser.add_argument("--dry-run", action="store_true", help="Perform dry-run without writing execution log resource result")
    
    # runtime-event-execution-log-registry コマンドパーサー
    runtime_event_execution_log_registry_parser = subparsers.add_parser("runtime-event-execution-log-registry", help="Manage plugin runtime session event execution log execution registry")
    runtime_event_execution_log_registry_parser.add_argument("--dry-run", action="store_true", help="Perform dry-run without writing execution log registry result")
    
    # runtime-event-execution-log-repository コマンドパーサー
    ...
    # runtime-execution-pipeline コマンドパーサー
    ...
    # runtime-execution-controller コマンドパーサー
    ...
    if args.command == "build":
    ...
    elif args.command == "runtime-event-execution-log-provider":
        run_runtime_event_execution_log_provider(args)
    elif args.command == "runtime-event-execution-log-instance":
        run_runtime_event_execution_log_instance(args)
    elif args.command == "runtime-event-execution-log-session":
    ...
    elif args.command == "runtime-event-execution-log-resource":
        run_runtime_event_execution_log_resource(args)
    elif args.command == "runtime-event-execution-log-registry":
        ...
        run_runtime_event_execution_engine(args)
    elif args.command == "runtime-execution-runtime":

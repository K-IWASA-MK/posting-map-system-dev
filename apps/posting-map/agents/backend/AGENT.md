...
- **スプシ非表示データベース設計の徹底**:
  - 管理者にスプレッドシート（生データ）を一切渡さない（見せない）ため、今後スマホ側で新しくカードや機能（例：スマホ側でのカード新規作成・登録機能など）を構築する際は、スプレッドシート上に新しいシートタブを増やして表示させるのではなく、**非表示のデータベースシートに行データとして裏で非表示処理する設計**を徹底すること。

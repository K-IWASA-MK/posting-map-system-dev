# AI Workforce Constitution v1.1.0
System: POSTING MAP / FIELD OPERATIONS OS
Author: 岩佐CEO
Status: STABLE RELEASE (Version 1.1.0)
...
### 5. Data Lineage Standard v1.0 (データ血統標準 - NEW)
- 全 Production AI が生成する正本データに共通の `lineage` オブジェクトを義務付け、全工程のデータ由来・依存バージョン・生成時刻を100%完全追跡可能とする。
- **共通 `lineage` 構造**:
  ```json
  {
    "lineage": {
      "producer": "[Production AI 名]",
      "sourceArtifact": "[参照した元ファイル名/データソース]",
      "sourceVersion": "[参照元のバージョン]",
      "generatedAt": "[ISO8601 生成日時]",

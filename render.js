      ...
      return `
        <div class="stock-row flex flex-col pt-1 pb-4 border-b border-white/5 last:border-b-0 rounded-xl px-2 -mx-2 gap-2"
          ...
          <div class="flex items-center justify-center w-full py-1" style="gap: 32px;">
            ...
            <button type="button" onclick="var r=this.closest('.stock-row');if(window.openTransferRequestDialog){window.openTransferRequestDialog(r.dataset.name,r.dataset.id,r.dataset.loc,parseFloat(r.dataset.count)||0);}else{alert('システムが準備中です。');}" style="background: rgba(6,199,85,0.1); border-color: rgba(6,199,85,0.3); color: #06C755; gap: 6px;" class="flex items-center justify-center px-5 py-1.5 rounded-full border transition-all active:opacity-70">

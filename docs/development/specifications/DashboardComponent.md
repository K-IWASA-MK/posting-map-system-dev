...
1. **Top KPI 統合性 & 時系列（Temporal）インジケータ**:
   - 4大 KPI ユニットは `ExecutiveKPICard` をコンテナとして横一列に整列配置し、レスポンシブ配置を行う。
   - `ExecutiveKPIElement` は、現在値に加えて、比較過去値（`Previous`）、増減率（`▲ / ▼`）、および静的分類された重要度ステータスラベル（`NORMAL` / `HIGH` / `SIGNIFICANT`）を描画する。
   - 増減率は、変化方向（`trendDirection`）に基づき、緑色（UP・正常）、黄色（変化中）、赤色（急変・警告）、または灰色（安定）の配色を用いて視覚的な強調を行い、タッチ操作用UIは一切配置しない。
...
## ナレッジレイヤーコンポーネント規則 (Knowledge Layer Component Rules)
ナレッジコンポーネント（`EventKnowledgeCard`、`EventKnowledgeItem`）は以下の規則に従う。
1. **完全表示専用 (View Only)**:
   イベントの要約・整理を表示するのみとし、操作用のボタン、フォーム、入力用 select/input は一切設置しないこと。
2. **不変属性 (Immutable Props) のバインド**:
   ナレッジストアから取得したナレッジ配列を不変データとして描画し、コンポーネント内部でデータの並び替えや改変は行わないこと。
3. **AI推論判定（No AI Judgment）の排除**:
   コンポーネント内部、および関連ビルダー・アダプターにおいて、AI判定や自律的トラブル解決等のロジックを一切含まない表示専用とすること。

---

## インサイトレイヤーコンポーネント規則 (Insight Layer Component Rules)
インサイトコンポーネント（`EventInsightCard`、`EventInsightItem`）は以下の規則に従う。
1. **完全表示専用 (View Only)**:
   インサイト統計情報を表示するのみとし、承認や実行、設定変更などの操作ボタン、入力要素（input, select, form）は一切配置しないこと。
2. **不変属性 (Immutable Props) のバインド**:
   インサイトストアから取得したインサイト配列を不変データとしてそのまま描画し、コンポーネント内部で並び替えや改変は行わないこと。
3. **推奨・予測（No Prediction / No Recommendation）の排除**:
   コンポーネント内およびビルダーにおける、AIによる予測判定や推奨アクション生成、自動対応指示などのロジックは一切排除すること。

---

## エボリューションレイヤーコンポーネント規則 (Evolution Layer Component Rules)
エボリューションコンポーネント（`EventEvolutionCard`、`EventEvolutionItem`）は以下の規則に従う。
1. **完全表示専用 (View Only)**:
   前後状態の差分や構造変化履歴を表示するのみとし、操作用のボタン、フォーム、入力要素（input, select, form）は一切配置しないこと。
2. **不変属性 (Immutable Props) のバインド**:
   エボリューションストアから取得した差分配列を不変データとしてそのまま描画し、コンポーネント内部で並び替えや改変は行わないこと。
3. **自動改善・異常判定（No Reasoning）の排除**:
   コンポーネント内、およびビルダー・アダプターにおける、AIによる原因分析、異常判定、改善提案、および自律自動対応ロジックは一切排除すること。

---

## パターンレイヤーコンポーネント規則 (Pattern Layer Component Rules)
パターンコンポーネント（`EventPatternCard`、`EventPatternItem`）は以下の規則に従う。
1. **完全表示専用 (View Only)**:
   繰り返し発生する構造パターンの統計情報を表示するのみとし、操作用のボタン、フォーム、入力要素（input, select, form）は一切配置しないこと。
2. **不変属性 (Immutable Props) のバインド**:
   パターンストアから取得したパターン配列を不変データとしてそのまま描画し、コンポーネント内部で並び替えや改変は行わないこと。
3. **AI学習・予測（No Learning）の排除**:
   コンポーネント内、およびビルダー・アダプターにおける、AIによるモデルの動的学習、将来発生の予測判定などのインテリジェンスロジックは一切排除すること。

---

## メモリレイヤーコンポーネント規則 (Memory Layer Component Rules)
メモリコンポーネント（`EventMemoryCard`、`EventMemoryItem`）は以下の規則に従う。
1. **完全表示専用 (View Only)**:
   長期履歴スナップショットを表示するのみとし、操作用のボタン、フォーム、入力要素（input, select, form）、および履歴削除・編集などの操作UIは一切配置しないこと。
2. **不変属性 (Immutable Props) のバインド**:
   メモリストアから取得したメモリ配列を不変データとしてそのまま描画し、コンポーネント内部で並び替えや改変は行わないこと。
3. **自己改善・AI予測（No Self-Improvement）の排除**:
   コンポーネント内、およびビルダー・アダプターにおける、自己改善計画の策定、モデル生成、将来予測判定などのロジックは一切排除すること。

---

## デモ可視化レイアウト規則 (Demo Visualization Layout Rules)
デモダッシュボード（AIOS Control Center）の構成要素は以下の規則に従う。
1. **2カラムグリッド配置の維持**:
   デスクトップなどの大画面においては、パイプラインの 8 レイヤーを 2 列構成に均等グリッド配置し、視認性を最大化すること。
2. **対話型操作要素の完全排除**:
   デモ画面内に、デモ開始・停止ボタン、模擬イベント手動発行用のフォーム、速度制御セレクトボックス等の操作UIを一切追加してはならない。
3. **独立デモソースデータバインドの徹底**:
   コンポーネントおよびレンダラーは EventBus の受信イベントをデータソースとし、表示層のコード内で模擬イベントを直接動的挿入するなどの責任逸脱をしてはならない。

...
## シミュレーション再開条件 (Simulation Resume Conditions)
統合シミュレーション環境においてテストシナリオが一時中断（例: `Approval Pending` シミュレートや意図的な `Mock Error`）された際、そのシミュレーションフローを再開する条件を以下の通り定義する。

- **検証対象**:
  - **Simulation ID**: 中断されたシミュレーションセッションの一意なID。
  - **Scenario 状態**: 中断時におけるテストシナリオの状態および中断要因がシミュレーション設定通りに解決されているか。
  - **Scope & Last Valid State**: シミュレーション再開対象の入力パラメータの型が元と一致していること、および直近に完了した模擬レイヤーの出力データ（ハッシュ）が改ざんなく引き継がれていること。
- **合格基準**: 上記の条件を全て検証し合格（Passed）した場合に限り、シミュレーターは中断された模擬レイヤーからシミュレーションフローを続行する。
```

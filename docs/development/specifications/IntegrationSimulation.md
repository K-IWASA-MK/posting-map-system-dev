...
## ダッシュボード観測フロー (Dashboard Observation Flow)
統合シミュレーションの稼働状況および最終結果レコードは、ダッシュボード上にリアルタイムまたは非同期で反映（マッピング）され、管理者が監視可能になる。
- **データ更新タイミング**:
  - `SimulationRuntime` が実行を終了し `AuditWriter` によって結果が追記されたタイミング、またはモックデータの更新時にダッシュボード側で読み込みがトリガーされる。
- **読み取り専用監視**:
  - ダッシュボードUIは模擬実行データのみを読み込み、UIからの再試行（Retry）や結果の上書き・消去、およびモック結果の本番環境への適用ボタンは一切マッピング・表示しない。
- **ダッシュボード・マッピングフロー (Dashboard Mapping Flow)**:
  - `Simulation KPI` -> `passed` (シミュレーション成功件数) / `failed` (シミュレーション失敗件数)。
  - `Simulation UI Badge` -> シミュレーション実行中（`Active`）/ 検証終了時（`Idle`）の稼働ステータスを可視化。

...
## イベント分類パターン監視規則 (Event Pattern Observer Layer)
ダッシュボードは、エボリューションデータから構造シグネチャを静的グループ化し、発生頻度や時系列分布を提示するパターン機能を提供する。
- **AI学習・予測の厳禁**: パターン分類機能は観測のみを担当し、AIによるモデルの動的学習、将来パターンの予測、異常予兆の検出、および自動対応は一切禁止する。
- **ストアによる不変パターン管理**: パターンオブジェクトは `Object.freeze` で不変保存し、最大 `300` 件までの容量制約を敷設し、超過時は古い順に破棄すること。
- **表示専用および操作UIの排除**: 設定変更やアクション実行などの対話型操作要素（ボタン、フォーム、入力用 select/input）は一切配置しないこと。

## イベント履歴メモリ監視規則 (Event Memory Observer Layer)
ダッシュボードは、抽出されたパターンおよびエボリューション履歴データを長期履歴スナップショットとして蓄積するメモリ機能を提供する。
- **自己改善・予測の厳禁**: メモリ機能は履歴の保存・参照のみを担当し、AIによる自己改善計画の策定、モデル生成、将来予測、および Kernel 操作などのWrite操作は一切禁止する。
- **ストアによる不変スナップショット保持**: メモリオブジェクトは `Object.freeze` で不変保存し、最大 `1000` 件までの容量制限を敷設し、超過時は古い順に破棄すること。
- **表示専用および操作要素の排除**: アーカイブの削除や編集、再適用などの操作用 UI（ボタン、入力欄、フォーム）は一切配置しないこと。

## デモ可視化監視規則 (AIOS Dashboard Demo Visualization)
ダッシュボードは、現在存在する AIOS Intelligence Pipeline 全体の価値を可視化するための統合デモ画面を提供する。
- **独立デモプロバイダー分離の原則**: ダッシュボードの表示レイヤー自身が模擬的なデモイベントを直接生成・発行してはならない。模擬イベントは完全に独立した Event Provider (`DemoEventGenerator.js`) から EventBus を介して非同期的に受信し、表示のみを駆動させること。
- **Observer Boundaryの完全順守**: パイプラインデータフローの可視化のみを目的とし、設定変更や模擬操作のトリガー用 UI（ボタン、フォーム、入力要素）は一切追加してはならない。

## エグゼクティブデモ可視化監視規則 (AIOS Dashboard Executive Demo Visualization)
ダッシュボードは、8つのインテリジェンスレイヤーの生データをマクロに集約し、経営・顧客向けに高度に要約された統合デモ画面（Executive View）を提供する。
- **二画面共存および共用設計**: 既存の 8 レイヤー生データ監視ビュー（Raw View）を破壊・削除することなく再利用可能な状態で維持し、URLのクエリパラメータ（例: `?view=raw` または `?view=executive` デフォルトは executive）を通じて表示モードのみを安全に切り替えること。
- **マクロ集約と静的テキストルール要約**: 技術的なイベントログをビジネス向けの表現に要約する際は、静的なマッピング定義に基づくルール変換に厳しく限定すること。AIによる障害判定や自律的意思決定、および「対応してください」のようなリスク推奨メッセージの動的生成は一切行ってはならない。
- **フロー循環構造の提示**: イベントが知識資産へ変換されていく循環構造を `Intelligence Flow Graph` で静的に提示し、Observer Boundary の Read-Only 原則を順守すること。
